...
static void start_gui_updater() {
    if (update_timer != 0) g_source_remove(update_timer);
...
static void on_start_clicked(GtkButton *btn, gpointer d) { start();set_buttons_state(STATE_RUNNING);}
static void on_stop_clicked(GtkButton *btn, gpointer d) { stop(); set_buttons_state(STATE_PAUSED);}
static void on_step_clicked(GtkButton *btn, gpointer d) { step(); }
static void on_finish_clicked(GtkButton *btn, gpointer d) { finish();set_buttons_state(STATE_FINISHED);}
...
static void create_simulation_window(int algorithm, int quantum) {
    ...
    char info[256];
    GtkWidget *proc_count = gtk_label_new("");
    sprintf(info, "Process Count: %d", getProcessCount());
    gtk_label_set_text(GTK_LABEL(proc_count), info);
    gtk_box_pack_start(GTK_BOX(left_box), proc_count, FALSE, FALSE, 0);
    ...
    // Ready Queues
    for (int i = 0; i < 4; i++) {
        GtkWidget *frame = gtk_frame_new(i == 0 ? "Ready Queue 1" :
                                         i == 1 ? "Ready Queue 2" :
                                         i == 2 ? "Ready Queue 3" : "Ready Queue 4");
...
static void update_gui() {
    ...
    int index = gtk_combo_box_get_active(GTK_COMBO_BOX(ui.proc_info.dropdown)) + 1;
    ...
    // Populate rows
    for (int i = 0; i < 60; i++) {
        if (strlen(mem[i].Name) == 0 && strlen(mem[i].Data) == 0) continue;
